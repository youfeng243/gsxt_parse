...
from config.conf import mongo_db_webpage_old
...
def main():
    ...
    source_table_cursor = webpage_db_old.db[source_table].find({'priority': {'$ne': 0}}, no_cursor_timeout=True).batch_size(
        10000)
    ...
    for item in source_table_cursor:
        try:
            ...
            if search_name == replace_name_1 \
                ...
                webpage_db_old.insert_batch_data(source_table, [item])
